...
%
	...
	LEMO-00 connectors are used for input connection. The inputs are AC-coupled and have an impedance of 50\,Ω. 
	...
	can be generated using the TiGer timing pattern generator. See section \ref{cp:tiger} for details on the TiGer. 
	%
		\begin{figure*}[ht]
			\begin{center}
				\ifxHPTDC{
				...
				\caption{Schematic view of \adeviceName\ Gen\,1 board showing the inter-board connectors.\label{fig:schematics}}
			\end{center} 
		\end{figure*}
	%
    \ifxHPTDC{
	...
	Connector J2 is a coax clock input that must receive a 10~MHz clock if multiple boards are used together as described in section \ref{multiboard}.}{
    %TT4 & xTDC4
    Furthermore, for Gen\,1 boards three inter-board connectors can be found near the top edge of the \deviceName\ board, as displayed in Figure \ref{fig:schematics}. 
	Connector J25 is reserved for future use. The pinout of connector J12 is shown in Table \ref{J12} and the pinout of connector J6 is depicted in Table \ref{J6}. \textbf{Gen\,2 boards do not posses these three connectors}.
    }
...
\ifxHPTDC{
	\section{Synchronizing multiple boards}
		...
		\subsection{Connecting multiple boards}
			The boards must each receive a common \SI{10}{\mega\hertz} clock on connector J2. The connector is inside the PC enclosure. 
