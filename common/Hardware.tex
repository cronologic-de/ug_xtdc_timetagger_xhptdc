    ...
    \begin{figure*}[hb]
        \begin{center}
            ...
            \caption{
                ...
                bracket.%
                \ifxHPTDC{
                    Note, the TRG connector only triggers the ADC channel,
                    not the TDC channels (see Section~\ref{adc}).
                }{}
                \label{fig:bracket}
            }
        \end{center}
...
\ifxHPTDC{
    \section{Synchronizing multiple boards}
        \label{multiboard}
        If more than eight TDC inputs are required, up to eight boards can be
        synchronized within a system. 
        
        The \deviceName\ API described in Chapter~\ref{cp:api} manages up to
        eight boards automatically and provides a single data stream that
        ...
        \subsection{Crates for multiple boards}
            Most PC mainboards don't have enough PCIe slots to support eight
