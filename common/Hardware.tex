...
\ifxHPTDC{}{
    ...
    For a detailed overview of the hardware of the TBT-variant, please refer to our respective \href{https://docs.cronologic.de/desktopsolutions/}{user guide}.

    \begin{figure}[htb]
        \centering
        \includegraphics[width=0.9\textwidth]{figures/xTDC4-TBT_xTDC4_PCIe.png}
        \caption{Overview of the PCIe (left) and the TBT (right) variant of the \deviceName.}
...
\section{\deviceName\ Inputs and Connectors}
    ...
    \begin{figure*}[htb]
        \centering
            ...
            \caption{
                Input circuit for each of the input channels.
                \label{fig:inputcirc}
            }
    \end{figure*}
    %
    ...
    in Figure \ref{fig:inputcirc}.  The digital threshold for each input can be
    adjusted individually to comply with a multitude of single-ended signaling
    standards. The threshold can also be used to configure the input for either
    ...
    \ifxHPTDC{
        ...
        use. The pinout of connector J12 is shown in Table~\ref{J12}.
        ...
        Furthermore, for \textbf{Gen\,1} boards three inter-board connectors 
        can be found near the top edge of the \deviceName\ board, as displayed 
        in Figure~\ref{fig:schematics}. Connector J25 is reserved for future 
        use. The pinout of connector J12 is shown in Table~\ref{J12}.
        \textbf{Gen\,2 boards do not possess these connectors}.
    }
    %
        \begin{figure*}[htb]
            \centering
                ...
                \caption{Schematic view of \adeviceName\ 
                    board including the inter-board connectors.
                    \ifxHPTDC{}{The inter-board connectors are only present
                    on Gen\,1 boards.}
...
\ifxHPTDC{
    ...
    \begin{itemize}
        ...
        \item On the PCB (see Figure~\ref{fig:schematics}): The red LED2 lights up during the configuration of the FPGA and turns off afterward. If it stays permanently lit, the configuration failed. The green LED1 lights up after the board is initialized.
    \end{itemize}
}
{
