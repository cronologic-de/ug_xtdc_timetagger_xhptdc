...
You must tell your compiler to link with the file \ifxHPTDC{\textsf{xhptdc8\tu driver.lib} 
or the corresponding 64 bit version \textsf{xhptdc8\tu driver\tu 64.lib}.}{\textsf{xTDC4\tu driver\tu 64.lib}.}
...
\ttdef{OK 0}\\
Error codes are set by the API functions to this value if there has been no error. Other error codes can be found in \textsf{\txh{TimeTagger4}{xTDC4}{xHPTDC8}\tu interface.h}\par
...
\section{Driver Information}
	...
	\ttvar{int}{count\tu devices(}\cronvar{int}{*error\tu code}, \cronvar{char}{**error\tu message)}\label{countdevices}\\
	Pointers to an error code variable to an error code and variable have to be provided. If error\tu code does not equal \ttdef{OK} = 0, the error message will contain what went wrong. Example crono kernel was not properly installed. Returns the number of boards present in the system that are supported by this driver.\par
...
\section {Initialization}
		...
		\ifxHPTDC{boards in the system}{board with the given index}. \\
		...
		The parameter \textsf{params} is a pointer to a structure of type \textsf{\initparameters} that must be completely initialized by get\tu default\tu init\tu parameters\par
...
%%%%%%%%%%%%%%%%% struct init_parameters
		...
		\subsection{Structure \initparameters}
			...
			\cronvar{uint64\tu t}{buffer\tu size\ifxHPTDC{}{[8]}}\\
			...
			\ifxHPTDC{}{
				...
				\cronvar{uint64\tu t}{buffer\tu address}\\
			...
			The update delay of the write pointer after a packet has been sent over PCIe. Specified in multiples of \SI{16}{\nano\second}.
			...
			\ifxHPTDC{
				...
				If set to 1 use external \SI{10}{\mega\hertz} reference. If set to 0 use internal reference.\par	
			}
	...
	\section{Configuration}
		...
		\ttvar{int}{get\tu current\tu configuration(}\deviceconfig,  \cronvar{\prefix \ifxHPTDC{manager\tu }{}configuration} \lb{\textbf{*config})}\\
		Gets current configuration. Copies the current configuration to the specified config pointer.\par

		\ttvar{int}{get\tu default\tu configuration(}\deviceconfig, \cronvar{\prefix \ifxHPTDC{manager\tu }{}configuration}\lb{\textbf{*config})}\\
	...
	\ifxHPTDC{
		...
		\label{yaml}
		There exist a community maintained utility library for the \deviceName\ that contains a convenience function that can modify configuration structures from a YAML config string.
		This can significantly shorten code required to setup the TDC. \\*
		See \href{https://github.com/cronologic-de/xhptdc8_babel/tree/main/util}{github.com/cronologic-de/xhptdc8\tu babel/tree/main/util} for details.  
	}{} 
	...
	\ifxHPTDC{
	   ...
	   There is a 64~kByte flash memory on each board that users can utilize to store any type of data. 
	   ...
	   We recommend to always allocate a buffer of the size of the flash memory given by \textsf{\PREFIX FLASH\tu SIZE} to clarify that always the full buffer is overwritten.\par
	}{}
