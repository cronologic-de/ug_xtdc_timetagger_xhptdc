...
\ifxHPTDC{
    ...
    \begin{description}[style=nextline]
        ...
        \item[\ttdef{TRIGGER\tu COUNT 16}]
        The number of potential trigger sources for the timing generators. One for
        each TDC input, one for the Start input plus some specials.  See
...
\begin{description}[style=nextline]
    ...
    \item[\cronvar{int}{dma\tu read\tu delay}]
    ...
    \ifxHPTDC{
        \item[\cronvar{crono\tu bool\tu t}{multiboard}]
...
\begin{description}[style=nextline]
    ...
    \item[\protect{\parbox[b]{0.9\linewidth}{
...
\ifxHPTDC{
    ...
    \begin{description}[style=nextline]
        ...
        \item[\protect{\parbox[b]{1.0\linewidth}{
            \ttvar{int}{read\tu user\tu flash(\device, \cronvar{int}{index},\\
                \hspace*{\labelwidth+\itemsep}\cronvar{\uchar *}{flash\tu data}, \cronvar{\uint}{size})}\\
            \ttvar{int}{write\tu user\tu flash(\device, \cronvar{int}{index},\\
                \hspace*{\labelwidth+\itemsep}\cronvar{\uchar *}{flash\tu data}, \cronvar{\uint}{size})}
        }}]
