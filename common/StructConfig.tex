...
\begin{description}[style=nextline]
    ...
    \ifxHPTDC{}{
        \textbf{Important Note:} The supported range changed for \hyperref[sec:rev_driver_applications]{driver release} 1.10.0. That means, if you use a value for \texttt{dc\_offset} outside the new supported range in your source code, the device configuration will report an error. Adjust your \texttt{dc\_offset} accordingly, preferably using the defines provided below.
    }
