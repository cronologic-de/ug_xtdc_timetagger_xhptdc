\ifxHPTDC{
	...
	\ttvar{\prefix device\tu configuration}{device\tu configurations[\PREFIX DEVICES\tu MAX]}\\
	...
	\ttvar{\prefix grouping\tu configuration}{grouping}\\
...
\subsection{Structure \prefix \ifxHPTDC{device\tu}{}configuration}
	...
	\ifxHPTDC{
		...
		Defaults to \textsf{\PREFIX TDC\tu MODE\tu CONTINUOUS} 	
...
%%%%%%%%% trigger
	...
	\txh{
		...
		Even though the TDC can only capture either reither or falling edges but not both, the gating blocks and the TiGer are more flexibel. 
		Set the corresponding flag for one of the edges or both edges then using the input with a TiGer or gating block.
	}\par
...
%%%%% tiger_block			
	...
	\ifxHPTDC{
		...
		\begin{tabular}{lrl}
			...
			\ttdef{TIGER \tu BIPOLAR} & 2 & Output is driven wuth 1V bidirectional pulses.\\*
									&   & $start = stop -1$\\*
		\end{tabular}
		The gating blocks are only used intenally.		
...
\ifxHPTDC{
	\subsection{Structure \prefix grouping}	
	...
	Is disabled per default by setting this paramteer to $-1$.\par
	...
	Values in the interval from \textsf{start} to \textsf{stop} are included in the group. 
	Either or both values can be negative to create common-stop behaviour. 
	\[
		-2^{63} \le start \le stop < 2^{63} 
	\]\par
	...
	\cronvar{crono\tu bool\tu t}{allow\tu overlap}\\
